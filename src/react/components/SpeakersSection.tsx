...
export function SpeakersSection() {
  ...
  const firstBlockSpeakers: Speaker[] = [
    {
  ...
  const secondBlockSpeakers: Speaker[] = [
    {
  ...
  const inauguralSpeaker: Speaker = {
    id: "jose-villela",
    name: "José Villela",
  ...
  return (
    <section id="conferencistas" className="py-20 bg-white relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        >
          ...
          <div className="flex justify-center">
            <motion.div
              ...
              className="bg-white rounded-2xl p-8 shadow-lg hover:shadow-xl transition-all duration-300 border border-gray-100 group max-w-md"
            >
              <div className="w-32 h-32 mx-auto mb-6 rounded-full bg-gradient-to-br from-golden-100 to-golden-200 flex items-center justify-center text-golden-600 text-2xl font-bold shadow-md group-hover:shadow-lg transition-shadow duration-300">
                {inauguralSpeaker.imageUrl ? (
                  <img 
                    src={inauguralSpeaker.imageUrl} 
                    alt={inauguralSpeaker.name}
              ...
              <div className="text-gray-600 leading-relaxed text-sm">
                <motion.div
                  initial={false}
                  animate={{ 
                    height: expandedSpeakers.has(inauguralSpeaker.id) ? "auto" : "auto"
                  }}
                  transition={{ duration: 0.3, ease: "easeInOut" }}
                >
                  <p className={expandedSpeakers.has(inauguralSpeaker.id) ? "" : "line-clamp-6"}>
                    {inauguralSpeaker.bio}
                  </p>
                </motion.div>
                <motion.button
                  onClick={() => toggleBioExpansion(inauguralSpeaker.id)}
                  ...
                  whileTap={{ scale: 0.98 }}
                >
                  {expandedSpeakers.has(inauguralSpeaker.id) ? (
                    <>
                      Ver menos
        ...
        >
          <h3 className="text-3xl md:text-4xl font-bold text-gray-900 mb-8 text-center">
            Primer Bloque - Conferencias Simultáneas
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {firstBlockSpeakers.map((speaker, index) => (
              <motion.div
                key={speaker.id}
                initial={{ opacity: 0, y: 50 }}
                whileInView={{ opacity: 1, y: 0 }}
        ...
        >
          <h3 className="text-3xl md:text-4xl font-bold text-gray-900 mb-8 text-center">
            Segundo Bloque - Talleres Simultáneos
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {secondBlockSpeakers.map((speaker, index) => (
              <motion.div
                key={speaker.id}
                initial={{ opacity: 0, y: 50 }}
                whileInView={{ opacity: 1, y: 0 }}
